...
const createTask = async (req, res) => {
    const { title,complete, important, range, icon, tags, taskDate } = req.body
    if (!title) return res.status(404).json({ message: 'title is required' })
    const task = await Task.create({ title, complete, important, range, icon, tags, taskDate })
    if (task) return res.status(201).json(task)
    else res.status(400).json({ message: 'Invalid task' })
}

const updateTask = async (req, res) => {
    const { id, title, important, tags, taskDate, icon } = req.body
